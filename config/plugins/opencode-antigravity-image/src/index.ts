...
import type { GenerateImageInput, Content, InlineDataPart, TextPart, Account, AccountsConfig } from "./types";
import {
  loadAccounts,
...
async function tryFetchQuotaInfo(token: string, account: Account, config: AccountsConfig): Promise<string> {
  try {
    const cached = account.cachedImageQuota;
    const stale = !cached || (Date.now() - cached.updatedAt > QUOTA_CACHE_TTL_MS);
    if (stale) {
      const quota = await fetchImageModelQuota(token);
      if (quota) {
        await updateAccountQuota(config, account, quota.remainingFraction, quota.resetTime);
      }
    }
    const fresh = account.cachedImageQuota;
    if (fresh) {
      const pct = (fresh.remainingFraction * 100).toFixed(1);
      const resetStr = fresh.resetTime
        ? ` | Reseta: ${new Date(fresh.resetTime).toLocaleString("pt-BR", { timeZone: "UTC" })} UTC`
        : "";
      return `\n\n**📊 Quota:** ${pct}% restante${resetStr}`;
    }
  } catch {}
  return "";
}
...
Uses credentials from opencode-antigravity-auth.`,
        ...
        async execute(args, _context) {
          ...
          const getProjectId = (acct: Account) => acct.managedProjectId || acct.projectId || "rising-fact-p41fc";
                ...
                if (nextAccount) {
                  try {
                    ...
                    if (isCapacityError(retryError)) {
                      ...
                      const quotaMsg = await tryFetchQuotaInfo(effectiveToken, effectiveAccount, config);
                      return `❌ **Model capacity exhausted**

All accounts hitting capacity limits. Retry after: ${wait}${quotaMsg}`;
                    }
                    const message = retryError instanceof Error ? retryError.message : String(retryError);
